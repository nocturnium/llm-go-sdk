...
|---|---|---|---|---|---|---|
| OpenAI | `gpt-image-2` sync b64 + SSE partials | `sora-2[-pro]` async `/v1/videos` to MP4 bytes | `gpt-4o-mini-tts`, bytes | `gpt-transcribe`, multipart, SSE | OAI (canonical) | **P0** |
...
| Gemini API | `gemini-3.1-flash-image` via `generateContent` `responseModalities`; compat `/v1beta/openai/images/generations` | Veo 3.1 `:predictLongRunning` + operation poll + Files download; compat `/v1beta/openai/videos` | `gemini-3.1-flash-tts-preview` to PCM s16le 24 kHz | `gemini-3.5-transcribe` (Interactions API only) | native + partial OAI | **P3** |
...
- **Async is the norm for video and common for images.** 14 of 16 dedicated providers use
  create to handle to poll. Terminal vocabularies all map onto
...
provider-reported `MediaUsage.Cost` to `MediaPricing` to `ok=false` (never a guessed zero, same rule as
...
| D3 | **ElevenLabs provider** | `pkg/providers/elevenlabs` native: TTS (+`/stream` to `StreamSpeech`, `/with-timestamps` to `Alignment`), STT `scribe_v2` multipart, SFX and music as `Synthesize` with model routing, flows image/video as `ImageGenerator`/`VideoGenerator` with 402 to `ErrPlanRequired`; `character-cost` header to `MediaUsage`; `EnvElevenLabsAPIKey`; package doc justifies native transport per `AGENTS.md` step 2b | `ELEVENLABS_API_KEY` (present) | `TestLiveElevenLabs_Synthesize` (`eleven_flash_v2_5`, cheapest), `_StreamSpeech`, `_Transcribe`, `_SoundEffect`; flows image test skips on 402; **before coding, probe `GET /v1/flows/image/{id}` since the poll path is unverified** |
| D4 | **Gemini media** | native image via `generateContent` `responseModalities`, Veo 3.1 LRO to `VideoJob` (Files download, 2 d TTL), TTS PCM to `SpeechResponse` with `Format` set to `pcm_s16le/24000`, `gemini-3.5-transcribe` via Interactions; pricing rows; skip Imagen | `GEMINI_API_KEY` (**absent from `.env`**) | `TestLiveGemini_GenerateImage`, `_Synthesize`, `_Transcribe`; Veo gated separately |
...
D0 to D1 are sequential. D2, D3, D4 are independent after D1 and can run in parallel. D5 after D1.
...
- **Verified 2026-09-05 by probe** (was unverified in research):
  ...
  - Mistral TTS: `POST /v1/audio/speech` JSON `{model, input, voice_id|ref_audio, response_format
    ...
    `stream:true` (event-stream, event schema undocumented); moderation to 403.
