...
func TestMediaCapabilities(t *testing.T) {
	...
	caps := c.Capabilities()
	if caps.ImageGeneration != false || caps.VideoGeneration != false || caps.Speech != true || caps.Transcription != true {
...
func TestMediaSpeech(t *testing.T) {
	c := mediaTestClient(t, func(w http.ResponseWriter, r *http.Request) {
		...
		fmt.Fprint(w, `{"audio_data":"UklGRg=="}`)
	})
...
func TestMediaTranscription(t *testing.T) {
	c := mediaTestClient(t, func(w http.ResponseWriter, r *http.Request) {
		...
		if r.FormValue("model") != providerConfig.DefaultTranscriptionModel {
