...
func (c *Client) transcribe(ctx context.Context, route string, audio llms.MediaInput, o *llms.TranscribeOptions) (*llms.Transcription, error) {
	...
	if audio.URL != "" {
		return nil, c.mediaError(fmt.Errorf("inline audio required: %w", llms.ErrInvalidParameters))
	}
	extensions := map[string]string{"audio/wav": ".wav", "audio/mpeg": ".mp3", "audio/mp3": ".mp3"}
	...
	format := "json"
	if o.Diarize || o.WordTimestamps || o.Language != "" {
		return nil, c.mediaError(llms.ErrInvalidParameters)
	}
	...
	if fields["stream"] == "true" || format != "json" {
		return nil, c.mediaError(llms.ErrInvalidParameters)
	}
