...
// Option is a function that configures an Ollama client.
type Option func(*options)

// options contains configuration options for the Ollama client.
type options struct {
	...
	// Defaults to true for Ollama since it targets http://localhost.
...
func defaultOptions() *options {
	return &options{
		...
		// Ollama runs locally over plain HTTP by default, so relax SSRF
...
// WithAPIKey sets the API key (optional for remote Ollama servers).
