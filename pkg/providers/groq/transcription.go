...
func (c *Client) transcribe(ctx context.Context, route string, audio llms.MediaInput, o *llms.TranscribeOptions) (*llms.Transcription, error) {
	...
	if audio.FileID != "" || len(audio.Data) > 25*1024*1024 {
	...
	if fields["stream"] == "true" || (format != "json" && format != "verbose_json" && format != "text") {
		return nil, c.mediaError(llms.ErrInvalidParameters)
	}
	...
	if format == "text" {
		return &llms.Transcription{Text: string(raw), Model: fields["model"]}, nil
	}
	var wire struct {
...
// Translate translates uploaded audio to English through /audio/translations.
// It accepts the same options and returns the same errors as Transcribe.
func (c *Client) Translate(ctx context.Context, audio llms.MediaInput, opts ...llms.TranscribeOption) (*llms.Transcription, error) {
	return c.transcribe(ctx, "audio/translations", audio, llms.ApplyTranscribeOptions(opts...))
}
