...
**Scope**: This applies to all code in the module root `github.com/nocturnium/llm-go-sdk/v5` (this repository), including providers, internal packages, and core interfaces.
...
**Canonical location**: All provider code lives in `pkg/providers/<name>/`. This is the only provider location — import providers as `github.com/nocturnium/llm-go-sdk/v5/pkg/providers/<name>`. (Earlier builds had a top-level `providers/*` backwards-compat shim and `pkg/*` alias packages; both have been removed. The shared types now live only in the root `llms` package.)
